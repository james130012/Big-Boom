...
class Api:
    ...
    def _inject_comments_by_line_numbers(self, original_html, definitions):
        ...
        for defi in sorted_definitions:
            ...
            current_end_comment_tag = end_comment_tag_to_insert + ('\n' if not end_comment_tag_to_insert.endswith('\n') else '')
            if end_line_1based < len(lines):
                lines.insert(end_line_1based, current_end_comment_tag)
            else: 
                lines.append(current_end_comment_tag)
    ...
    def analyze_html(self, original_code):
        ...
        if len(current_active_module_definitions) != num_to_process and num_to_process > 0 : 
