...
# 定义用于LLM提示词的基础模板
PROMPT_TEMPLATE_BASE = """你是一个专业的Web前端开发助手。请分析以下HTML代码，识别并提取逻辑上独立、可复用的模块，特别关注图表相关的部分。

**输出要求**：

- 格式：JSON对象，包含以下字段：
  - "module_count_suggestion": 建议的模块数量（整数，最大10）。
  - "definitions": 模块定义的数组，每个模块包含：
    - "id": 模块ID（字符串，使用英文，避免通用名称，如 "module_1"）。
    - "description": 模块的中文描述（字符串）。
    - "start_comment": 模块起始标记，格式为 "LLM_MODULE_START: [模块ID]"。
    - "end_comment": 模块结束标记，格式为 "LLM_MODULE_END: [模块ID]"。
    - "start_line": 模块起始行号（整数，从1开始）。
    - "end_line": 模块结束行号（整数）。
    - "start_char": 模块起始字符索引（整数，从0开始）。
    - "end_char": 模块结束字符索引（整数）。
    - "priority_level": 模块优先级（字符串，"high"、"medium"或"low"）。图表相关模块应标记为 "high"。
    - "code_snippet_preview": 模块内关键代码的简短预览（字符串，最多50字符）。
...
- 不要返回模块的完整HTML内容。
- 不要包含多余的文本或Markdown代码块（如 ```json）。
- 输出必须是一个JSON对象，包含 "module_count_suggestion" 和 "definitions" 字段。
