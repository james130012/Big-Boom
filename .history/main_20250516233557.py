...
4. 优先识别图表相关模块（如包含 <canvas>、<svg>、ECharts、Chart.js 等内容），其 ID 应包含 "chart" 关键字，并将 priority_level 标记为 "high"。
5. 每个模块增加优先级字段：
   - "priority_level": "high" | "medium" | "low"。图表相关模块默认 "high"。
6. 为方便识别和管理，模块ID建议包含位置和用途（如 "main_chart_sales"），避免使用笼统名称。
7. 提供每个模块的中文描述，描述其功能或内容。
8. 定义清晰的起始和结束标记：
...
10. 可选提供模块内关键代码的简短预览：
    - "code_snippet_preview": "<HTML代码片段，最多50字符，用于快速识别>"。
11. 建议一个优先关注的模块数量 (module_count_suggestion)，不超过10个，基于主要模块的数量。
12. 确保模块定义逻辑独立，避免过细的拆分（如单独的标题或段落，除非有明确语义）。
13. 不要返回模块的完整HTML内容，不要包含多余的文本或Markdown代码块（```json）。
14. 输出必须是一个对象，包含 "module_count_suggestion" 和 "definitions" 字段，不允许直接返回数组。
  ...
  "definitions": [
    {
      ...
      "priority_level": "<模块优先级 (字符串)>",
      "code_snippet_preview": "<HTML代码片段预览 (字符串)>"
    }
  ]
}
