...
```
""" # End of PROMPT_TEMPLATE_BASE

DEFAULT_API_CONFIG = {
    "api_url": "https://openrouter.ai/api/v1/chat/completions",
    "default_model": "google/gemini-2.5-flash-preview", # Consider "openai/gpt-3.5-turbo" if issues persist with Gemini's JSON adherence
    "base_headers": {"Content-Type": "application/json"},
    "request_timeout_seconds": 180, # Increased timeout
    "llm_temperature": 0.05, # Lower temperature for more deterministic JSON output
    "llm_max_tokens": 8192, # CRITICAL: Increased max_tokens
...
class Api:
    ...
    def _add_comments_to_html_revised(self, original_html, definitions_from_llm):
        ...
        for i, d_orig in enumerate(definitions_from_llm):
            ...
            # RESTORED: Use proper HTML comment syntax
    ...
    def _extract_module_content_from_string(self, html_string_with_llm_comments, module_def):
        ...
        # RESTORED: Use proper HTML comment syntax for searching
    ...
    def _generate_skeleton_from_string(self, html_string_with_llm_comments, modules_to_skeletonize):
        ...
        for module_def in sorted_modules:
            ...
            # RESTORED: Use proper HTML comment syntax for searching for the block to replace
            full_start_marker = f"\n"
            full_end_marker = f"\n"
            # RESTORED: Use proper placeholder format
            placeholder = f"" 
    ...
    def analyze_html(self, original_code, specific_instruction=""):
        ...
        definition_prompt = f"""请分析以下HTML代码，识别其中主要的、功能独立的模块区域。
...
- "start_comment": "LLM_MODULE_START: <ID>" (请使用你生成的ID)
- "end_comment": "LLM_MODULE_END: <ID>" (请使用你生成的ID)
...
JSON输出（确保包含 "definitions" 键，并且 "start_char", "end_char" 精确包围模块内容）：
"""

        raw_definitions_from_llm = []
        if not self.openrouter_api_key:
            logging.warning("OPENROUTER_API_KEY is not set. Using mock data for definitions.")
            mock_defs_container = {"definitions": [ 
                {"id": "header_block", "description": "整个页眉区域", "start_char": 450, "end_char": 600, "start_comment": "LLM_MODULE_START: header_block", "end_comment": "LLM_MODULE_END: header_block"},
                {"id": "animation_area_1", "description": "动画1的完整容器和相关元素", "start_char": 700, "end_char": 950, "start_comment": "LLM_MODULE_START: animation_area_1", "end_comment": "LLM_MODULE_END: animation_area_1"},
            ]}
            raw_definitions_from_llm = mock_defs_container.get("definitions", [])
        else:
            try:
                ...
                payload = {"model": self.api_config.get("default_model"), "messages": [{"role": "user", "content": definition_prompt}], "temperature": self.api_config.get("llm_temperature"), "max_tokens": self.api_config.get("llm_max_tokens"), "response_format": {"type": "json_object"}}
                response = requests.post(self.api_config.get("api_url"), headers=headers, json=payload, timeout=self.api_config.get("request_timeout_seconds"))
                response.raise_for_status()
                raw_llm_def_text = response.json()["choices"][0]["message"]["content"]
                logging.debug(f"LLM RAW DEFINITION RESPONSE TEXT (first 1000 chars): {raw_llm_def_text[:1000]}")
                ...
                if isinstance(llm_response_data, list):
                    raw_definitions_from_llm = llm_response_data
                elif isinstance(llm_response_data, dict):
                    raw_definitions_from_llm = llm_response_data.get("definitions", [])
                else:
                    logging.error(f"LLM definition response is not a list or dict: {type(llm_response_data)}")
                    raw_definitions_from_llm = []
                logging.info(f"LLM processed {len(raw_definitions_from_llm)} module definitions.")
            except json.JSONDecodeError as json_e:
                logging.error(f"JSONDecodeError parsing LLM definition response: {json_e}")
                logging.error(f"Problematic text for definitions JSON parsing (first 500 chars): {cleaned_def_text[:500] if 'cleaned_def_text' in locals() else 'N/A'}")
                return {"status": "error", "message": f"LLM返回的定义不是有效的JSON格式: {json_e}", "active_module_definitions": [], "html_skeleton": "", "modified_code": {}, "modification_manual": ""}
            except Exception as e:
        ...
        if specific_instruction:
            # ... (LLM call for modification - same as v9/v10, with robust error handling) ...
            ...
            if not self.openrouter_api_key:
                ...
                try:
                    ...
                    headers = {"Authorization": f"Bearer {self.openrouter_api_key}", "Content-Type": "application/json", "HTTP-Referer": self.site_url, "X-Title": self.site_name}
                    payload = {"model": self.api_config.get("default_model"), "messages": [{"role": "user", "content": modification_prompt}], "temperature": self.api_config.get("llm_temperature"), "max_tokens": self.api_config.get("llm_max_tokens"), "response_format": {"type": "json_object"}}
                    response = requests.post(self.api_config.get("api_url"), headers=headers, json=payload, timeout=self.api_config.get("request_timeout_seconds"))
                    response.raise_for_status()
                    raw_llm_mod_text = ""
                    try:
                        ...
                        raw_llm_mod_text = response.text 
                    logging.info(f"LLM RAW MODIFICATION RESPONSE TEXT (first 1000 chars): {raw_llm_mod_text[:1000]}...")
    ...
    def integrate_modules_with_user_edits(self, user_edited_modules_json_string="{}"):
        ...
        for module_def in self.module_definitions:
            ...
            # RESTORED: Ensure placeholder matches what _generate_skeleton_from_string creates
            placeholder = f""
