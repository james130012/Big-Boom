...
class Api:
    ...
    def _add_comments_to_html_revised(self, original_html, definitions_from_llm):
        # Function to add LLM-specific comments robustly
        ...
        for i, d_orig in enumerate(definitions_from_llm):
            ...
            if not all([module_id, start_comment_text, end_comment_text]):
                logging.warning(f"Module '{module_id}' in _add_comments_to_html_revised: incomplete. Skipping.")
                continue
            s_char, e_char = d.get("start_char"), d.get("end_char")
            if s_char is None or e_char is None: # Fallback to lines if chars not present
            ...
            if not (0 <= s_char <= e_char <= len(original_html)):
                logging.warning(f"Module '{module_id}' invalid char positions ({s_char}-{e_char}). Skipping.")
                continue
    ...
    def _extract_module_content_from_string(self, html_string_with_llm_comments, module_def):
        # Extracts content between the LLM comment markers
        ...
        if not start_comment_text or not end_comment_text: return ""
        effective_start_marker = f"\n"
        effective_end_marker_prefix = f"\n"
        start_idx = html_string_with_llm_comments.find(effective_start_marker)
        if start_idx == -1: return None # Indicate failure to find start marker
        ...
        if content_end_idx == -1: return None # Indicate failure to find end marker
    ...
    def _generate_skeleton_from_string(self, html_string_with_llm_comments, modules_to_skeletonize):
        # Generates HTML skeleton with placeholders
        skeleton = html_string_with_llm_comments
        temp_modules = []
        for m_def in modules_to_skeletonize:
            m_copy = m_def.copy()
            if "s_char_final" not in m_copy: m_copy["s_char_final"] = m_copy.get("start_char", 0)
        ...
        for module_def in sorted_modules:
            ...
            if end_idx_of_end_marker == -1: continue
    ...
    def analyze_html(self, original_code, specific_instruction=""):
        ...
        # Using a simplified prompt for definitions to ensure LLM focuses on identification
...
"""
        ...
        if not self.openrouter_api_key:
            ...
            # Ensure mock data has start_char, end_char, start_comment, end_comment
            mock_defs = [
                ...
                {"id": "anim1_box_content", "description": "动画1的内容区", "start_char": 800, "end_char": 830, "start_comment": "LLM_MODULE_START: anim1_box_content", "end_comment": "LLM_MODULE_END: anim1_box_content"},
                {"id": "footer_section", "description": "页脚", "start_char": 1118, "end_char": 1174, "start_comment": "LLM_MODULE_START: footer_section", "end_comment": "LLM_MODULE_END: footer_section"}
            ] # Adjust char numbers based on the actual default HTML in gui.html
            ...
            try:
                ...
                if not cleaned_def_text:
                    raise ValueError("LLM returned empty content for definitions.")
                llm_response_data = json.loads(cleaned_def_text)
                raw_definitions_from_llm = llm_response_data.get("definitions", []) # Assuming LLM returns {"definitions": [...]}
                ...
                try:
                    ...
                    try {
                        ...
                        if response.json().get("choices") and len(response.json()["choices"]) > 0:
                            ...
                            raise ValueError("LLM response structure for modification is not as expected (no choices).")
                    } except (KeyError, AttributeError, ValueError) as ke: # Catch issues with accessing parts of the response
                        logging.error(f"Error accessing LLM modification response content: {ke}. Full response: {response.text[:500]}") # Log part of the full response
                        raw_llm_mod_text = response.text # Fallback to full text if specific path fails
                        # This raw_llm_mod_text might not be JSON, so parsing below will likely fail, which is handled.

                    logging.info(f"LLM RAW MODIFICATION RESPONSE TEXT: {raw_llm_mod_text[:1000]}...") # Log first 1000 chars
                    ...
                    if llm_modification_response.get("status") == "success":
                        ...
                        if llm_reported_modified_modules and isinstance(llm_reported_modified_modules, list) and len(llm_reported_modified_modules) > 0:
                            ...
                            else: logging.warning("LLM modification response did not specify a target module ID.")
                        else: logging.warning("LLM modification response did not list any modules as being modified.")
                    else:
                        logging.error(f"LLM reported an error during modification: {llm_modification_response.get('message', 'Unknown error from LLM')}")
                        modification_manual_for_response = f"LLM修改失败: {llm_modification_response.get('message', 'Unknown error from LLM')}"
                
                except requests.exceptions.RequestException as req_e:
                    logging.error(f"HTTP Request Error calling LLM for modification: {req_e}")
                    modification_manual_for_response = f"LLM API请求错误 (修改): {req_e}"
                except Exception as e: # Catch any other unexpected errors during the process
                    logging.error(f"Unexpected error during LLM modification call or processing: {e}")
                    modification_manual_for_response = f"处理LLM修改时发生意外错误: {e}"
        
        return {
            "status": "success", # analyze_html itself succeeded, even if LLM mod failed
    ...
    def integrate_modules_with_user_edits(self, user_edited_modules_json_string="{}"):
        # ... (same as main_py_v5, this part should be okay if inputs are correct) ...
        ...
        for module_def in self.module_definitions:
            ...
            else: logging.warning(f"Placeholder for '{module_id}' not found in skeleton.")
