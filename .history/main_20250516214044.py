...
DEFAULT_API_CONFIG = {
    ...
    "default_model": "google/gemini-2.5-flash-preview", # 默认模型，可以被 .env 覆盖
...
class Api:
    ...
    def _extract_module_content_from_string(self, html_string, module_def):
        ...
        if not start_marker_text or not end_marker_text:
            print(f"DEBUG (_extract_module_content_from_string): 模块 '{module_id}' 缺少 start_comment 或 end_comment 文本。跳过提取。")
            return ""
    ...
    def _generate_skeleton_from_string(self, html_string, modules_to_skeletonize):
        ...
        for i, module_def in enumerate(modules_to_skeletonize):
            ...
            if not start_marker_text or not end_marker_text:
                print(f"DEBUG (_generate_skeleton_from_string): 模块 '{module_id}' 缺少 start_comment 或 end_comment 文本，用于骨架生成。跳过。")
                continue
