...
**说明**：
- 移除 `module_content_html`，新增 `start_line`, `end_line`, `start_char`, `end_char`。
- 提供行号（1-based）和字符索引（0-based）两种定位方式，字符索引作为可选字段以应对复杂情况。
- 明确禁止返回完整 HTML 内容，减轻 LLM 输出负担。
- 保留示例，确保 LLM 理解位置定义。

#### 2. 修改 Python 代码
我们需要更新以下函数，基于 LLM 提供的起止位置切分 HTML：

- `_add_comments_to_html`：根据 `start_line`/`end_line` 或 `start_char`/`end_char` 注入注释。
- `_extract_module_content_from_string`：保持基于注释标记提取，但优化嵌套模块处理。
- `analyze_html`：保留输入验证和 API 调用逻辑，更新模块处理部分。
