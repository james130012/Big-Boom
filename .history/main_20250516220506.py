...
class Api:
    ...
    def _inject_comments_by_line_numbers(self, original_html, definitions):
        ...
        for defi in definitions:
            ...
            start_comment_text = defi.get("start_comment_text") # 这是纯文本，例如 "LLM_MODULE_START: header"
            end_comment_text = defi.get("end_comment_text")     # 这是纯文本，例如 "LLM_MODULE_END: header"

            if not (isinstance(start_line, int) and isinstance(end_line, int) and \
                    start_comment_text and end_comment_text and \
                    1 <= start_line <= end_line <= len(lines)): # 确保行号在有效范围内
        ...
        for defi in sorted_definitions:
            ...
            # 所以要在第 end_line_1based 行 (0-indexed: end_line_1based-1) 的 *内容之后* 插入，
            # 实际上是插入到下一行的位置，即索引 end_line_1based
            
            # 确保原始行末尾有换行符，以保证注释在新的一行
            if end_line_1based -1 < len(lines) and not lines[end_line_1based - 1].endswith('\n'):
                lines[end_line_1based - 1] += '\n'
            
            # 插入结束标记到 end_line_1based 之后（即成为新的一行，或追加到最后）
            if end_line_1based < len(lines):
                lines.insert(end_line_1based, end_comment_tag_to_insert + ('\n' if not end_comment_tag_to_insert.endswith('\n') else ''))
            else: # 如果是最后一行，直接追加
                lines.append(end_comment_tag_to_insert + ('\n' if not end_comment_tag_to_insert.endswith('\n') else ''))


            # 插入开始标记: 在第 start_line_1based 行之前插入 (0-indexed: start_line_1based-1)
            lines.insert(start_line_1based - 1, start_comment_tag)
            
            print(f"DEBUG (_inject_comments_by_line_numbers): 已为模块 '{module_id}' 在原始行 {start_line_1based}-{end_line_1based} 附近注入注释。")
    ...
    def _generate_skeleton_from_string(self, html_string_with_comments, modules_to_skeletonize):
        ...
        for i, module_def in enumerate(modules_to_skeletonize):
            ...
            while True:
                start_idx_current_loop = skeleton.find(start_marker, current_search_pos)
                if start_idx_current_loop == -1:
                    break 

                # 确定实际模块内容的开始（跳过起始标记和其后的换行符）
                content_block_starts_after_marker = start_idx_current_loop + len(start_marker)
                if skeleton[content_block_starts_after_marker:].startswith('\n'):
                    content_block_starts_after_marker += 1
                
                # 查找结束标记
                end_idx_current_loop = skeleton.find(end_marker, content_block_starts_after_marker)
                if end_idx_current_loop == -1:
                    current_search_pos = start_idx_current_loop + len(start_marker) # 避免死循环
                    continue
                
                # 确定要替换的完整块：从起始标记的开始 到 结束标记的结束
                block_to_replace = skeleton[start_idx_current_loop : end_idx_current_loop + len(end_marker)]
                
                skeleton = skeleton.replace(block_to_replace, placeholder, 1)
                break # 处理完当前模块定义，跳出while，处理下一个module_def
    ...
    def analyze_html(self, original_code):
        ...
        html_with_injected_comments = raw_original_code 
        ...
        if len(current_active_module_definitions) != num_to_process and num_to_process > 0 : # Avoid warning if num_to_process was 0
            print(f"警告: 某些模块定义缺少必要的 start/end_comment_text，已从处理列表中移除。预期处理 {num_to_process}，实际处理 {len(current_active_module_definitions)}")
        ...
        for module_def in current_active_module_definitions:
            ...
            # 将LLM返回的行号等信息也加入到processed_active_definitions，以便前端可能需要时使用
            processed_def_to_send = {**module_def, "original_content": content}
            # 移除 module_content_html 如果它存在于原始的 LLM 定义中（在新逻辑下不再需要发送给前端）
            processed_def_to_send.pop("module_content_html", None) 
            processed_active_definitions.append(processed_def_to_send)
        print("--- 结束处理模块定义 ---\n")
