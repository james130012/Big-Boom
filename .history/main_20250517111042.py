...
class Api:
    ...
    def analyze_html(self, original_code, specific_instruction=""):
        ...
        # This prompt asks for a specific JSON structure including a "definitions" key.
        definition_prompt = f"""分析以下HTML代码，严格按照JSON格式返回模块定义列表。JSON对象应包含一个名为 "definitions" 的键，其值为模块定义数组。每个模块包含：
...
"""
        ...
        if not self.openrouter_api_key:
            ...
            try:
                ...
                # MODIFICATION START: Handle if LLM returns a list directly or a dict with "definitions"
                if isinstance(llm_response_data, list):
                    raw_definitions_from_llm = llm_response_data
                    logging.info("LLM returned a direct list of definitions.")
                elif isinstance(llm_response_data, dict):
                    raw_definitions_from_llm = llm_response_data.get("definitions", [])
                    logging.info("LLM returned a dictionary, extracting 'definitions' key.")
                else:
                    logging.error(f"LLM definition response is neither a list nor a dict: {type(llm_response_data)}")
                    raw_definitions_from_llm = []
                # MODIFICATION END
